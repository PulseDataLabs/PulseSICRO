...
import pandas as pd
from utils.base import salvar_csv, agora_brt, get_logger
...
class BaseScraper:
    ...
    def run(self) -> None:
        ...
        if not is_pipeline:
            print(f"\n--- {self.title or self.name.replace('_', ' ').title()} ---")
        ...
        try:
            ...
            if not is_pipeline:
                print(f"Sucesso: {len(df_cleaned)} registros salvos em {self.output_file.name} ({elapsed:.1f}s)")
